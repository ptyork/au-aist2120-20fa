...
def add_contact():
    ...
    print()
    print('=== ADD A CONTACT ===')
    email = get_string("Enter the email address: ")
    email = email.lower()
    if email in contacts:
        print("CAN'T ADD A DUPLICATE WITH THAT ADDRESS")
        return
...
def display_all_contacts():
    ...
    print()
    print('=== ALL CONTACTS ===')
    print(f"{'Email':20}{'Name':30}{'Phone':>15}{'Birthday':>15}")
    print("=" * 80)
    for email in contacts:
        ...
        phone = parts[1]
        bday = parts[2]
        print(f"{email:20}{name:30}{phone:>15}{bday:>15}")
    print('-' * 80)
...
def delete_contact():
    global contacts
    print()
    email = input('Who to delete: ')
    if email.lower() not in contacts:
        print(f"{email} not in the list")
        return
    
    deleted = contacts.pop(email)  # del contacts[email]
...
def edit_contact():
    ...
    print()
    email = input('Who to edit: ')
    if email.lower() not in contacts:
        print(f"{email} not in the list")
        return

    edit_ctc = contacts[email]
    ...
    del contacts[email]  # DELETE THE OLD
    contacts[new_email] = f"{new_name},{new_phone},{new_bday}"  # ADD THE NEW
